...
async fn test_frequency_separation() -> UshResult<()> {
    ...
    for (i, (freq_0, freq_1)) in configs.iter().enumerate() {
        ...
        if !messages.is_empty() {
            ...
            if decoded_text == test_message {
                println!(
                    "✓ Frequency config {}: {:.0}Hz/{:.0}Hz - SUCCESS",
                    i + 1,
                    freq_0,
                    freq_1
                );
                ...
                println!(
                    "⚠ Frequency config {}: {:.0}Hz/{:.0}Hz - DECODE ERROR",
                    i + 1,
                    freq_0,
                    freq_1
                );
            ...
            println!(
                "⚠ Frequency config {}: {:.0}Hz/{:.0}Hz - NO MESSAGES",
                i + 1,
                freq_0,
                freq_1
            );
...
async fn test_performance_benchmarks() -> UshResult<()> {
    ...
    println!(
        "  Total roundtrip: {:?}",
        protocol_encode_time
            + modulation_encode_time
            + modulation_decode_time
            + protocol_decode_time
    );
...
async fn test_unicode_support() -> UshResult<()> {
    ...
    for (i, message) in unicode_messages.iter().enumerate() {
        ...
        assert!(
            !messages.is_empty(),
            "No messages decoded for Unicode test {}",
            i
        );
        let decoded_text = messages[0].get_text()?;
