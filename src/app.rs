...
use ush::audio::{AudioManager, AudioConfig};
use ush::modulation::{FskModulator, FskDemodulator, ModulationConfig, detect_signal_start, apply_bandpass_filter};
use ush::protocol::{ProtocolEncoder, ProtocolDecoder, Message, MessageType};
...
impl UshApp {
    ...
    pub async fn send_message(
        ...
        for i in 0..repeat_count {
            if i > 0 {
                ...
                let silence_samples = (self.settings.sample_rate as f32 * silence_duration) as usize;
                full_samples.extend(vec![0.0; silence_samples]);
            }
            full_samples.extend(&samples);
        }
        
        info!("Playing {} samples ({:.2}s) {} time(s)", 
              samples.len(), samples.len() as f32 / self.settings.sample_rate as f32, repeat_count);
    ...
    pub async fn listen_for_messages(
        ...
        if let Some(wav_path) = from_wav {
            ...
            return self.process_received_samples(&samples, filter, threshold).await;
        }
        ...
        let input_stream = self.audio_manager.create_input_stream(move |data| {
            ...
            if samples.len() > 44100 * 10 { // 10 seconds of audio
        ...
        loop {
            ...
            if last_process_time.elapsed() > Duration::from_millis(100) {
                ...
                if !samples.is_empty() {
                    if let Some(start_idx) = detect_signal_start(&samples, threshold) {
                        ...
                        if let Err(e) = self.process_received_samples(signal_samples, filter, threshold).await {
            ...
            if event::poll(Duration::from_millis(50)).unwrap_or(false) {
                if let Ok(Event::Key(key_event)) = event::read() {
                    if key_event.kind == KeyEventKind::Press && key_event.code == KeyCode::Char('c') && key_event.modifiers.contains(event::KeyModifiers::CONTROL) {
    ...
    async fn process_received_samples(&self, samples: &[f32], filter: bool, _threshold: f32) -> UshResult<()> {
        let processed_samples = if filter {
            info!("Applying bandpass filter");
            apply_bandpass_filter(samples, self.settings.freq_0 - 1000.0, self.settings.freq_1 + 1000.0, self.settings.sample_rate)
        } else {
            samples.to_vec()
        };
    ...
    async fn handle_received_message(&self, message: &Message) -> UshResult<()> {
        match &message.header.message_type {
            ...
            MessageType::Ping => {
                println!("Received ping from sequence {}", message.header.sequence_number);
            }
            MessageType::Ack => {
                println!("Received ACK for sequence {}", message.header.sequence_number);
            }
            MessageType::File => {
                println!("Received file chunk (sequence {})", message.header.sequence_number);
            }
        }
                        ...
                        match key_event.code {
                            ...
                            KeyCode::Enter => {
                                if !input_buffer.trim().is_empty() {
                                    ...
                                    if let Err(e) = self.send_message(&message, None, None, None).await {
    ...
    pub async fn send_file(&self, file_path: &Path, chunk_size: Option<usize>, delay: Option<u64>) -> UshResult<()> {
        let chunk_size = chunk_size.unwrap_or(64);
        let delay_ms = delay.unwrap_or(500);
        
        info!("Sending file: {:?} (chunk size: {} bytes, delay: {}ms)", file_path, chunk_size, delay_ms);
        ...
        loop {
            ...
            println!("Sent chunk {} ({}/{} bytes, {:.1}%)", 
                     sequence - 1, bytes_sent, file_size, 
        ...
        // Send end marker
        let end_message = format!("FILE:{}:END", file_path.file_name().unwrap().to_string_lossy());
        self.send_message(&end_message, Some(1), None, None).await?;
        
        println!("File transfer complete: {} bytes in {} chunks", bytes_sent, sequence);
        Ok(())
    }
    
    pub async fn receive_file(&self, output_path: &Path, _timeout_secs: Option<u32>) -> UshResult<()> {
    ...
    pub async fn run_test(&self, test_type: &TestCommands) -> UshResult<()> {
        match test_type {
            ...
            TestCommands::Generate { frequency, duration } => {
    ...
    async fn list_audio_devices(&self) -> UshResult<()> {
        ...
        println!("This feature requires additional implementation for device enumeration.");
        Ok(())
    }
    ...
    pub async fn debug_mode(&self, spectrum: bool, waveform: bool, rate: Option<u32>) -> UshResult<()> {
        let update_rate = rate.unwrap_or(10);
        println!("Debug mode - spectrum: {}, waveform: {}, rate: {}Hz", spectrum, waveform, update_rate);
    ...
    fn load_wav_file(&self, path: &Path) -> UshResult<Vec<f32>> {
        ...
        let samples: UshResult<Vec<f32>> = match spec.sample_format {
            hound::SampleFormat::Float => {
                reader.samples::<f32>().map(|s| s.map_err(UshError::from)).collect()
            }
            hound::SampleFormat::Int => {
                match spec.bits_per_sample {
                    ...
                    32 => {
                        ...
                        Ok(samples?.into_iter().map(|s| s as f32 / i32::MAX as f32).collect())
                    }
