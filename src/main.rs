...
async fn main() -> UshResult<()> {
    ...
    if settings.freq_0 >= settings.freq_1 {
        return Err(UshError::Config {
            message: format!("freq_0 ({}) must be less than freq_1 ({})", settings.freq_0, settings.freq_1),
        });
    }
    
    info!("Starting ush v0.1.0");
    info!("Audio settings: {}Hz sample rate, freq_0={}Hz, freq_1={}Hz", 
          settings.sample_rate, settings.freq_0, settings.freq_1);
    
    match &cli.command {
        Commands::Send { message, repeat, save_wav, from_wav } => {
            let app = UshApp::new(settings)?;
            app.send_message(message, *repeat, save_wav.as_deref(), from_wav.as_deref()).await
        }
        Commands::Listen { timeout, save_wav, from_wav, filter, threshold } => {
            let app = UshApp::new(settings)?;
            let threshold = threshold.map(validate_threshold).transpose()
                .map_err(|e| UshError::Config { message: e })?.unwrap_or(0.1);
            app.listen_for_messages(*timeout, save_wav.as_deref(), from_wav.as_deref(), *filter, threshold).await
        }
        Commands::Chat { username, ack, timeout } => {
            let app = UshApp::new(settings)?;
            app.start_chat_mode(username.as_deref(), *ack, *timeout).await
        }
        Commands::SendFile { file, chunk_size, delay } => {
            let app = UshApp::new(settings)?;
        ...
        Commands::Debug { spectrum, waveform, rate } => {
            let app = UshApp::new(settings)?;
